...
pub fn parseEnv(allocator: std.mem.Allocator, io: std.Io, path: []const u8) !*Cfg {
    ...
    const content = allocator.alloc(
        ...
        std.log.err(
            "Failed to allocate memory for env file: {any}",
            .{err},
        );
        return config;
    };
    ...
    while (lines.next()) |line| {
        ...
        if (std.mem.eql(u8, key, "PORT")) {
            ...
            config.redis_url = try allocator.dupe(
                u8,
                val,
            );
        } else if (std.mem.eql(u8, key, "KAFKA_BROKERS")) {
            config.kafka_brokers = try allocator.dupe(
                u8,
                val,
            );
        } else if (std.mem.eql(u8, key, "ROCKSDB_PATH")) {
            config.rocksdb_path = try allocator.dupe(
                u8,
                val,
            );
        }
