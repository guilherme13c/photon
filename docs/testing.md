...
`make test-fast` is the PR-fast gate: unit tests, versioned Kafka contracts,
deterministic scheduler simulation, and short fuzzing. `make test-integration`
runs the Kafka-backed Go service integration tests. `make test-functional`
starts the complete Compose stack with the `test` profile and only crawls the
controlled fixture origin.
